...
def has_permission(user_role, required_permissions):
    """
    Verifica si un rol tiene los permisos requeridos
    """
    role_permissions = {
        'administrador': ['all'],  # Admin tiene todos los permisos
        'registro': ['clientes', 'choferes', 'vendedores', 'inventario', 'despachos'],  # Registro hereda funciones del admin
        'ensayo': ['inventario', 'alertas']  # Ensayo solo inventario y alertas
    }
    ...
    # Verificar si tiene los permisos específicos requeridos
...
# API para clientes - Modificada para verificar permisos
@app.route('/api/clientes', methods=['GET'])
def get_clientes():
  if 'user_role' not in session or not has_permission(session['user_role'], 'clientes'):
...
# API para choferes - Modificada para verificar permisos
@app.route('/api/choferes', methods=['GET'])
def get_choferes():
  if 'user_role' not in session or not has_permission(session['user_role'], 'choferes'):
      return jsonify({'error': 'Sin permisos'}), 403
...
# API para vendedores - Modificada para verificar permisos
@app.route('/api/vendedores', methods=['GET'])
def get_vendedores():
  if 'user_role' not in session or not has_permission(session['user_role'], 'vendedores'):
      return jsonify({'error': 'Sin permisos'}), 403
...
def add_despacho():
  if 'user_role' not in session or not has_permission(session['user_role'], 'despachos'):
      flash('Sin permisos para esta acción', 'error')
      return redirect(url_for('dashboard', role=session.get('user_role', 'admin')))
...
# API para inventario - Modificada para verificar permisos
@app.route('/api/inventario', methods=['GET'])
def get_inventario():
  if 'user_role' not in session or not has_permission(session['user_role'], 'inventario'):
      return jsonify({'error': 'Sin permisos'}), 403
